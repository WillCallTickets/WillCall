...
// INVOICES.js
//
// database access methods for the invoices table
////////////////////////////////////////////////////////////
...
module.exports.saveCart = function(input){
  ...
  if(cart.idx) {
    ...
    .then(function (data) {
      if (data) {
        ...
        return knex('invoices').insert({
          ...
          status: 'inprocess',
          balancedue: cart.items.reduce(function (prev, cur) {
    ...
    .then(function (invoice_id) {
      ...
      // now save individual invoice items - attach to invoice_id
      var _date = Date.now();
      ...
      cart.items.forEach(function (itm) {
        itemPromises.push(
          knex('invoiceitems').insert({
            ...
            description: itm.secondaryDesc.reduce(function (prev, cur) {
              return prev += ' ' + cur;
            }, '').trim(),
            price: itm.itemPrice,
            quantity: itm.qty,
